      ...
      >
        ...
        <div className="flex items-center justify-between w-full">
          ...
          <div className="hidden md:flex flex-1 items-center justify-center px-4 lg:px-8">
            <div className="flex items-center space-x-4 md:space-x-5 lg:space-x-7 xl:space-x-10">
              {navLinks.filter(link => link.name !== 'Contact').map((link) => {
                const isActive = link.href 
                  ? (link.href === '/about' && location.pathname === '/about') ||
                    (link.href.includes('/services/offerings') && location.pathname.includes('/services/offerings')) ||
                    (link.href === '/how-its-done' && location.pathname === '/how-its-done') ||
                    (link.href === '/blog' && location.pathname.startsWith('/blog'))
                  : (link.subLinks && (location.pathname.startsWith('/projects') || (location.pathname.startsWith('/services') && !location.pathname.includes('/services/offerings'))));

                if (link.subLinks) {
                  return (
                    <div key={link.name} className="group py-2">
                      <button 
                        className={`text-[11px] uppercase tracking-widest font-semibold transition-colors duration-300 flex items-center space-x-1.5 focus:outline-none cursor-pointer whitespace-nowrap ${
                          isActive 
                            ? 'text-brand-plum font-bold border-b-2 border-brand-plum/40 pb-0.5' 
                            : 'text-brand-charcoal/80 hover:text-brand-plum'
                        }`}
                      >
                        <span>{link.name}</span>
                        <svg className="w-3 h-3 transition-transform duration-300 group-hover:rotate-180 flex-shrink-0" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                ...
                return (
                  <a
                    ...
                    className={`text-[11px] uppercase tracking-widest font-semibold transition-colors duration-300 whitespace-nowrap ${
                      isActive 
                        ? 'text-brand-plum font-bold border-b-2 border-brand-plum/40 pb-0.5' 
                        : 'text-brand-charcoal/80 hover:text-brand-plum'
                    }`}
          ...
          <div className="flex-shrink-0 flex items-center justify-end space-x-4">
            ...
            <a
              ...
              className="hidden md:inline-flex items-center justify-center bg-brand-charcoal hover:bg-brand-plum text-white px-4 py-2 md:px-5 md:py-2.5 rounded-full text-[11px] font-semibold uppercase tracking-widest transition-all duration-300 hover:scale-[1.02] active:scale-[0.98] shadow-xxs hover:shadow-xs cursor-pointer select-none whitespace-nowrap"
            >
              Get in Touch
            </a>
